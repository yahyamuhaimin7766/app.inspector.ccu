  ...
  <form @submit.prevent="handleSubmit" class="p-4 md:p-6 space-y-6 max-w-xl mx-auto pb-28">
    ...
    <div class="space-y-1.5">
      ...
      <input type="date" v-model="form.tanggal" required
        class="w-full bg-white text-slate-900 border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-xs">
    </div>
    ...
    <div class="space-y-1.5">
        ...
        <div class="relative w-full h-[300px] bg-black rounded-xl overflow-hidden flex items-center justify-center">
          ...
          <div class="absolute inset-0 flex items-center justify-center pointer-events-none z-10 bg-black/40">
             <div class="w-[90%] h-[25%] border-2 border-red-500 rounded bg-transparent shadow-[0_0_0_999px_rgba(0,0,0,0.6)] relative">
                <span class="absolute -top-6 left-0 right-0 text-center text-[11px] text-white font-bold drop-shadow-md">
                  POSISIKAN <span class="text-red-400">VIN (MHK/PM2)</span> DI DALAM KOTAK
                </span>
      ...
      <div class="relative">
        <input type="text" v-model="form.no_rangka" placeholder="Scan OCR atau ketik..." required
    ...
    <!-- TIPE & VARIAN (Otomatis dari Master NIK) -->
    <div class="space-y-1.5">
      <label class="text-xs font-bold tracking-wider text-slate-600 uppercase">Tipe & Varian Kendaraan *</label>
      <div class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 shadow-inner flex items-center justify-between transition-all duration-300" 
        ...
        <span v-else-if="form.tipe === 'Tipe Tidak Ditemukan'" class="text-red-500 italic font-bold">
          ⚠ Tipe tidak ditemukan di Master NIK
        </span>
        <span v-else class="text-slate-400 italic text-sm">
          Terisi otomatis setelah scan/ketik VIN...
        </span>
    ...
    <div class="space-y-1.5">
      ...
      <div class="grid grid-cols-4 gap-2">
        <button v-for="w in options.warna" :key="w" type="button" @click="form.warna = w"
          :class="['py-2.5 px-2 text-xs font-bold rounded-xl border transition shadow-xs', 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="space-y-1.5">
        ...
        <input type="text" v-model="form.kode_accu" placeholder="Kode Accu"
    ...
    <div class="space-y-1.5">
      ...
      <div class="grid grid-cols-4 gap-2">
        <button v-for="qc in options.qc" :key="qc" type="button" @click="form.stempel_qc = qc"
          :class="['py-3 text-xs font-bold rounded-xl border transition shadow-xs', 
    ...
    <div class="space-y-1.5">
      ...
      <input type="text" v-model="form.defect" placeholder="Jenis kerusakannya..."
        class="w-full bg-white text-slate-900 placeholder-slate-400 border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-xs">
    </div>
    <div class="space-y-1.5">
      <label class="text-xs font-bold tracking-wider text-slate-600 uppercase">Keterangan Defect</label>
      <textarea v-model="form.ket_defect" rows="3" placeholder="Contoh: RR LH, bocor, terlipat"
        class="w-full bg-white text-slate-900 placeholder-slate-400 border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:outline-none transition shadow-xs"></textarea>
    </div>
...
// Fungsi OCR Cepat (Gambar di-scale 50% & kompresi 0.8)
const takeSnapshotAndRead = async () => {
  ...
  try {
    ...
    const base64CroppedImage = canvas.toDataURL('image/jpeg', 0.8);
    ...
    if (result && result.ParsedResults && result.ParsedResults.length > 0) {
      const scannedText = result.ParsedResults[0].ParsedText.replace(/\s+/g, '').toUpperCase();
      const vinRegex = /(MHK|PM2)[A-HJ-NPR-Z0-9]{14}/g;
...
const handleSubmit = () => {
  if(!form.tipe || form.tipe === 'Tipe Tidak Ditemukan') { 
    alert('No Rangka (VIN) tidak valid! Tipe Kendaraan harus terisi otomatis sebelum menyimpan.'); return; 
  }
  if(!form.warna) { alert('Pilih Warna!'); return; }
  if(!form.stempel_qc) { alert('Pilih Stempel QC!'); return; }
  emit('submit', { ...form });
};
